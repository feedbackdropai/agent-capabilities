...
The prompt may also include a `---` fenced overrides block (`code-standards`, `extra-code-standards`, `scripts`). Extract the path(s) and any overrides from the prompt. If only a `Plan:` line is present, no overview is available.
